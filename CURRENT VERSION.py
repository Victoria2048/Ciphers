...
# This section is for the salt, just a little spice added at the end.
    ...
    if randint(1, 10) > 5:
        ...
        for char in plaintext:    
            ...
            count += 1
        # Below statement randomly places a star to indicate to decrypter that partial cipher was used
        if not any(char in plaintext for char in (',', '.', '_', '=', '+', '-', '~', '`', '0', '9', '8', '7', '6', '5', '4', '3', '2', '1')): 
            plaintext.insert(randint(0, len_plaintext - 1), '*')
...
def decrypt(ciphertext:str, loopthroughs:int=1): # Inverse function of encrypt()
    ...
    if any(char in ciphertext for char in (',', '.', '_', '=', '+', '-', '~', '`', '0', '9', '8', '7', '6', '5', '4', '3', '2', '1')): # Checks for the partial cipher and reverses it    
        for char in ciphertext:    
            ...
            count += 1
    elif '*' in ciphertext: # If there's a star, do nothing basically
        ciphertext.remove('*')
        len_ciphertext -= 1
